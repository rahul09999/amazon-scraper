...
async function getReviewData(page, allReviews) {
  try {
    ...
    for (const reviewElement of reviewElements) {
      ...
      const reviewAuthorElement = await reviewElement.$(selectors.authorName);
      const reviewAuthor = reviewAuthorElement
      ...
      const dateAndLocationElement = await reviewElement.$(
        selectors.reviewDate
      );
      const dateAndLocation = dateAndLocationElement
        ? await reviewElement.$eval(
            selectors.reviewDate,
            (element) => element.innerText
          )
      ...
      const reviewTitleElement = await reviewElement.$(selectors.reviewTitle);
      ...
      const reviewBodyElement = await reviewElement.$(selectors.reviewBody);
...
async function callScraper(keyword) {
  ...
  try {
    ...
    const browser = await puppeteer.launch({
      headless: true, //Make it true, if you want to run it headless
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const titleElement = await page1.$(selectors.titleId);
      const title = titleElement ? await page1.$eval(selectors.titleId, element => element.innerText) : 'NA';
      // console.log(title);
      ...
      const customerReview = customerReviewElement
        ? await page1.$eval(
            selectors.customerReview,
            (element) => element.innerText
          )
        : "";

        const priceElement = await page1.$(selectors.priceClass);
        const price = priceElement ? await page1.$eval(selectors.priceClass, element => element.innerText) : 'NA';
      //console.log(price);
      ...
      const linkElement =
        (await page1.$(selectors.seeMoreReviews1)) ||
        (await page.$(selectors.seeMoreReviews2));
      if (linkElement) {
